...
export default {
  ...
  methods: {
    ...
    //     return
    //   }
    //   const { data, code } = await this.$apis.filterMaterial({
    //     ...e,
    ...
    //     this.pagiProps.total = data.total
    //     console.log(this.pagiProps)
    //   }
    // },
    // 格式化选择器时间
    // formatDate (arr) {
    ...
    async pageRequest (query) {
      // this.query = query
      // const { data, code, msg } = await this.$apis.filterMaterial(query)
      // if (code === 0) {
      //   this.filterData = data.data
      // } else {
      //   this.$message({
      ...
      //   data
      // }
      return this.$apis.filterMaterial(query)
    },
    delRequest (row) {
